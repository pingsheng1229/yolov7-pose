...
def detect_hand_raise(kpts):
    ...
    global left_arm_curl_count
    global right_arm_curl_count
    global left_arm_angle_prev 
    global right_arm_angle_prev 

    left_arm_angle = calculate_angle(left_shoulder, left_elbow, left_wrist)
    if kpts[17]>confidence and kpts[23]>confidence and kpts[29]>confidence:
        if left_arm_angle < 105 and left_arm_angle_prev >= 105:
    ...
    if kpts[20]>confidence and kpts[26]>confidence and kpts[32]>confidence:
        if right_arm_angle < 105 and right_arm_angle_prev >= 105:
    ...
    if (right_wrist[1] < right_shoulder[1] and
        right_elbow[1] < right_shoulder[1] and
        kpts[20]>confidence and kpts[26]>confidence and kpts[32]>confidence):
...
def display_hand_raise_status(im0, kpts):
    ...
    if status != "no raise hand":
        ...
        text_position = (nose_x - 40, nose_y - distance * 5)
...
class YOLO(object):
    _defaults = {
        ...
        #--------------------------------------------------------------------------#
        "model_path"        : 'C:\yolov7-pose\yolov7-tiny-pytorch-master\model_data\yolov7_tiny_weights.pth',
        "classes_path"      : 'C:\yolov7-pose\yolov7-tiny-pytorch-master\model_data\coco_classes.txt',
        #---------------------------------------------------------------------#
        #   anchors_path代表先验框对应的txt文件，一般不修改。
        #   anchors_mask用于帮助代码找到对应的先验框，一般不修改。
        #---------------------------------------------------------------------#
        "anchors_path"      : 'C:\yolov7-pose\yolov7-tiny-pytorch-master\model_data/yolo_anchors.txt',
    ...
    def detect_image(self, image, crop=False, count=False):
        ...
        for i, c in list(enumerate(top_label)):
            ...
            draw.rectangle([tuple(text_origin), (text_origin[0] + label_width, text_origin[1] + label_height)], fill=self.colors[c])
        ...
        for det in results:
            if det is not None:
                kpts = det[0, 6:]
                display_hand_raise_status(np.array(image), kpts)
